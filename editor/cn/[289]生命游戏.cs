    ...
    public void GameOfLife(int[][] board)
    {
        for (int i = 0; i < board.Length; i++) {
            for (int j = 0; j < board[0].Length; j++) {
                int liveNeighbors = Board(i - 1, j - 1) + Board(i - 1, j) + Board(i - 1, j + 1)
                                  + Board(i + 0, j - 1) /*             */ + Board(i + 0, j + 1)
                                  + Board(i + 1, j - 1) + Board(i + 1, j) + Board(i + 1, j + 1);
                if (board[i][j] == 1 && liveNeighbors is 2 or 3)
                    board[i][j] = 0b11;
