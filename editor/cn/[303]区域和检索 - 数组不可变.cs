...
// 
//
// Related Topics 设计 数组 前缀和 👍 711 👎 0
...
{
    private readonly List<int> _prefixSums; // _prefixSum[i] = sum[0..i), lazy
    ...
    {
        var k = right + 2 - _prefixSums.Count;
        for (int i = 0; i < k; i++) {
            _prefixSums.Add(_prefixSums[^1] + _nums[_prefixSums.Count - 1]);
        }
