...
public class Solution
{
    ...
    {
        var n = stones.Length;
        var dp = new BitArray[n]; // dp[i][j] 表示是否能跳到第 i 块石子，且跳距为 j
        for (int i = 0; i < n; i++) dp[i] = new(n + 1); // 跳距最大不会超过 n
        dp[0][0] = true;

        var has = new bool[n]; // has[i] 表示是否能跳到第 i 块石子
        has[0] = true;
        ...
        for (int i = 1; i < n; i++)
            for (int j = i - 1; j >= 0; j--) {
                ...
                // we always have 1 <= gap <= j + 1 <= n - 1, so gap - 1, gap, gap + 1 are all in [0, n]
                var canReach = dp[j][gap] || dp[j][gap - 1] || dp[j][gap + 1];
                if (canReach) {
                    dp[i][gap] = true;
                    has[i] = true;
                }
