...
// 
//
// Related Topics 深度优先搜索 图 欧拉回路 👍 988 👎 0
...
public class Solution
{
    public IList<string> FindItinerary(IList<IList<string>> tickets)
    {
        ...
        foreach (var ticket in tickets) {
            ...
            fromNode.Nexts.Enqueue(toNode, to);
        }
        ...
        void Dfs(Node node)
        {
            while (node.Nexts.Count > 0) {
                var next = node.Nexts.Dequeue();
                Dfs(next);
            }
            result.Add(node.Airport);
        }
    }

    private class Node(string airport)
    {
        public string Airport { get; } = airport;
        public PriorityQueue<Node, string> Nexts { get; } = new();
    }
}
