...
//输出：true
//解释：图片展示了节点间如何连接。白色节点向前跳跃，而红色节点向后跳跃。
//我们可以看到存在循环，按下标 0 --> 1 --> 0 --> ...，当它的大小大于 1 时，它有一个向前跳的节点和一个向后跳的节点，所以 它不是一个循
//环。
...
//
// Related Topics 数组 哈希表 双指针 👍 228 👎 0
...
public class Solution
{
    public bool CircularArrayLoop(int[] nums)
    {
        ...
        for (var i = 0; i < nums.Length; i++) {
            ...
            var add = i;
            while (nums[add] * nums[NextIndex(add)] > 0) {
                var temp = add;
                add = NextIndex(add);
                nums[temp] = 0;
            }
        ...
        int NextIndex(int index) => ((index + nums[index]) % n + n) % n;
    }
}
